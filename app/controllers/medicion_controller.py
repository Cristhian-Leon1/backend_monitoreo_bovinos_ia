...
from app.models.medicion_models import MedicionCreate, MedicionUpdate
...
class MedicionController:
    ...
    async def get_estadisticas_mediciones_bovino(self, bovino_id: str, propietario_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pesos = [float(m.get('peso', 0)) for m in mediciones if m.get('peso')]
            alturas = [float(m.get('altura', 0)) for m in mediciones if m.get('altura')]  
            longitudes = [float(m.get('longitud', 0)) for m in mediciones if m.get('longitud')]
            ...
            estadisticas = {
                ...
                "ultima_medicion": mediciones[0],  # Ya están ordenadas por fecha desc
                "primera_medicion": mediciones[-1]
            }
...
medicion_controller = MedicionController()
